# AGENTS.md

## 工作方式要求（最高优先级）

**用户明确要求：每做一件事情必须全面考虑，不要一步步做！**

- 🐱 每次对话开始必须先称呼"魔力猫"
- 💪 想尽一切办法完成任务，不找借口、不打折扣
- 📝 **只有在功能/代码修改已完成、已测试验证、可验收时，才更新 `docs/PROJECT_PROGRESS.md`**
- 📖 **每次开始新任务前，必须先阅读 `docs/PROJECT_PROGRESS.md`**
- 阅读代码时优先覆盖关键链路的完整上下文，不机械追求固定行数；通常单个关键文件应尽量读到 1000 行左右，或至少覆盖入口、核心逻辑、状态回写和测试。
- 你要积极使用 subagent，但只在任务可明确拆成互不冲突、边界清晰、输出独立的子问题时使用；不要为了并行而拆散本应由主线统一判断的问题。
- 验收时不要同时拉多套本地服务；先考虑 scheduler / worker 的副作用，避免后台放大 CPU。
- 任务结束前必须回收自己拉起的 dev/start/watch/worker 进程，并确认端口已释放。
- 研究类、讨论类、约束类文档，写在本地 `docs/design/` 或 `docs/research/`，**不要写进 `PROJECT_PROGRESS.md`**
- 除非用户明确要求，**不要顺手修改需求范围之外的代码、文档、配置或文案**
- 🌐 **浏览器自动化/截图优先使用 `bb-browser`，不要优先使用 Playwright**
  - 只要任务属于页面访问、交互、抓取、截图、DOM 检查、浏览器态验证，默认先用 `bb-browser`
  - **只有以下情况才允许回退到 Playwright**：
    1. 用户明确要求 Playwright
    2. `bb-browser` 在当前环境不可用
    3. 任务明确依赖 Playwright 特性（如 `@playwright/test` 测试套件、特定设备仿真、项目已有 Playwright 测试基建必须复用）
  - 如果用了 Playwright，必须在汇报里说明为什么没有用 `bb-browser`

### 🚨 端到端完成任务交付（强制要求）

1. **端到端完成** - 不要做一半让用户来测试/验收
2. **主动测试验证** - 完成功能后自己主动测试，不要让用户来测
3. **全面考虑** - 思考完整方案，不要一步步做让用户催
4. **主动汇报** - 验证通过后再告诉用户，不要说"应该没问题"

### 禁止行为（绝对禁止）
- ❌ **不要让用户来测试**
- ❌ **不要让用户来验收**
- ❌ **不要说"应该可以，你自己试试"**
- ❌ **不要做一半问用户怎么办**
- ❌ **不要等用户发现 bug 才去修复**

### 核心原则
> **宁可多花时间逐个文件确认，也不使用快捷参数盲修。**
> **用户的代码永远比我的时间更宝贵。**

### 变更边界锁定（强制）

开始实现前，必须先明确写出：

1. 本轮要修什么
2. 本轮不改什么
3. 允许修改的模块边界
4. 不允许触碰的主链路

如果用户明确要求“只修当前链路问题”，不得顺手升级为架构重构、模板回退、运行时重写或控制面重做。

### Bug 修复与架构重构分流（强制）

如果用户要求修复现有链路问题，默认按 bug-fix 处理，不自动升级为架构重构。

只有同时满足以下条件，才允许升级为架构级改造：

1. 现有链路无法在原边界内修复
2. 已给出明确根因说明
3. 已列出影响面和“不改”的替代方案
4. 已获得用户明确确认

### 分层修复原则（强制）

涉及自迭代、审批、发布、运行时链路时，必须先判断问题属于哪一层：

1. 执行层
2. 治理层
3. 展示层

如果根因在执行层，默认先不修改治理层和展示层。
不要用控制面和 UI 收口去掩盖执行链缺陷。

### Live Replay 使用规则

真实链路回放默认用于验收已确定方案，不用于边测试边决定架构方向。

如果方案边界尚未锁定：

1. 先做代码 review / 根因分析
2. 先写清楚改动边界
3. 再做 live replay

不要把 live replay 当成主要设计工具。

### 用户需求整理规范
- 整理用户需求/用户场景时，按模块或用户旅程分组，只写用户故事和 `[支持] / [不支持]`。
- 不写表格、ID、当前证据、优先级建议、可验收清单；除非用户明确要求。
- 不使用“部分支持”：没有形成完整可用闭环的场景统一记为 `[不支持]`，描述要尽量详细但简洁。

### 前端可见文案约束
- You should not use visible, in-app text to describe the application's features, functionality, keyboard shortcuts, styling, visual elements, or how to use the application.
- 不要把内部设计说明、结构解释、实现意图、交互引导语写成用户可见文案；界面上只保留业务事实、状态、风险、结果和当前动作。

### 完成任务的标准（必须满足）
每完成一个功能，必须：
1. ✅ 自己测试验证通过（有实际测试输出）
2. ✅ 确认端到端流程能跑通
3. ✅ 有测试证据（日志/截图/输出）
4. ✅ 再告诉用户完成情况

非编程任务（如需求整理、用户场景索引、研究/讨论/约束文档）不需要测试验证，也不需要为验证而启动服务；完成文档修改后直接汇报修改位置即可。

> 这是强制工作流程，避免重复劳动和信息脱节。


## 五、检查清单

### 任务开始前
- [ ] 称呼"魔力猫"
- [ ] **阅读 `docs/PROJECT_PROGRESS.md` 了解当前进度**
- [ ] 理解完整需求，阅读关联代码
- [ ] 设计完整方案，向用户确认

### 任务执行中
- [ ] 一次性完成所有修改
- [ ] **代码改了 → 同步更新相关文档**（见下方文档同步规则）
- [ ] **执行测试并全部通过**
- [ ] **重启相关服务**
- [ ] **验证健康检查**

### 任务完成后
- [ ] 有实际测试输出作为证据
- [ ] **仅在代码/配置/数据修改已完成且验证通过后，更新 `docs/PROJECT_PROGRESS.md`**
- [ ] 确认以上全部完成后，再汇报

## 六、文档同步规则（强制）

### 原则
> **代码改了 → 文档必须改。不是"之后再补"，是当场改。**

### `PROJECT_PROGRESS.md` 使用边界（强制）

`docs/PROJECT_PROGRESS.md` 只用于记录：

1. 已完成并通过验证的实现
2. 已完成并有验收证据的修复
3. 已落地的系统行为变化

不要写入：

1. 纯讨论
2. 方案推演
3. 约束说明
4. 研究笔记
5. 尚未落地、尚未验收的设计结论

这些内容应写到：

1. `docs/design/`
2. `docs/research/`

### 代码变更 → 文档动作

| 代码变更 | 必须同步更新的文档 |
|:---------|:-----------------|
| 新增/修改 `src/app/api/` route.ts | `gateway-api.md` + `cli-api-reference.md` + `ARCHITECTURE.md` API 表 |
| 新增 `src/lib/` 目录（新子系统） | `ARCHITECTURE.md` 新增章节 + 模块依赖图 + `agent-user-guide.md` 新增节 |
| 重构/拆分模块 | `ARCHITECTURE.md` 关键文件表 |
| 修改数据模型（types.ts） | `agent-user-guide.md` 字段表 |
| 新增配置项/环境变量 | `agent-user-guide.md` 配置示例 |

### 文档同步分级

代码改动后，只同步更新与本轮真实行为变化直接相关的文档。

1. 外部 API / 用户可见行为变化：更新 `guide + ARCHITECTURE`
2. 内部实现细节变化但外部行为不变：优先更新 `ARCHITECTURE` 或 `docs/design/`
3. 纯测试、重构、命名清理：默认不扩散更新多份用户文档

禁止为了文档整洁顺手扩大代码改动范围。

为了提高开发效率，等级非常低的描述性小错误，不做调整
