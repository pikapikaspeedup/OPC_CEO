...
> **实现更新（2026-04-16）**：当前前端已落地为“Header 一级主菜单 + Sidebar 二级上下文栏”的变体，而不是纯图标 Nav Rail。保留 `CEO Office / OPC / Chat / Knowledge / Ops` 五个一级入口在顶部，`Settings` 移入右上角工具区；左侧栏只显示当前模块的二级内容与上下文。

一级入口固定在 Header，左侧栏不再承担主菜单职责：

```
┌─ Header ─────────────────────────────────────────────────────────────┐
│  CEO Office │ OPC │ Chat │ Knowledge │ Ops      ...      Settings   │
└──────────────────────────────────────────────────────────────────────┘
┌─ Sidebar (Context Only) ─────────────────────────────────────────────┐
│ 当前模块说明 / 快捷动作 / 项目树 / 对话分组 / 知识条目 / Ops 资产       │
└──────────────────────────────────────────────────────────────────────┘
```

- Header 主菜单在桌面态允许横向滚动收缩，在移动端通过全屏 sheet 展开
- 左侧栏只保留一个主滚动容器，上方上下文头固定，下方列表滚动
- `Settings` 不再占据一级导航位
...
| Header Primary Nav | 顶部一级主菜单 | 替代 sidebar 的标签部分 |
