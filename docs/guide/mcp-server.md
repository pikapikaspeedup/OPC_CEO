...
>
> Split 模式说明：即使部署成 `web + control-plane + runtime`，MCP 对外工具契约也不变；MCP 仍然通过统一 Gateway 入口访问公开 API。
...
MCP Server 当前提供多项工具。这些能力经过封装，天然满足大语言模型的 Schema 调用。
...
### 7.1 antigravity_create_scheduler_job
创建定时任务，支持四种业务动作：
...
- `cronExpression / intervalMs / scheduledAt`

不同动作额外参数：

- `dispatch-pipeline`: `workspace`, `prompt`, `templateId`，可选 `stageId`, `projectId`
- `dispatch-prompt`: `workspace`, `prompt`，可选 `promptAssetRefs`, `skillHints`, `projectId`
- `health-check`: `projectId`
- `create-project`: `departmentWorkspaceUri`, `goal`，可选 `skillHint`, `createProjectTemplateId`

其中 `dispatch-prompt` 类型的 job 在触发时会以 Prompt Mode 执行——由 AI 按业务 prompt 主导完成任务，无需依赖模板。适合无法唯一确定模板但有明确执行意图的场景。`promptAssetRefs` 可引用 playbook/workflow 资产作为提示参考，`skillHints` 可指定技能倾向。

`create-project` 类型的 job 在触发时会先创建一个 Ad-hoc Project；如果提供了 `createProjectTemplateId`，还会在同一次触发里自动派发第一条 run。未提供该字段时，则只创建项目，不直接启动 run。

- **Hint**: `destructive`

### 7.2 antigravity_update_scheduler_job
更新现有定时任务。最常见用途是暂停或恢复：

- `enabled: false` → 暂停
- `enabled: true` → 恢复

也支持更新名称、调度周期和动作配置。更新动作配置时，可以显式传 `actionKind` 切换动作；如果保持原动作不变，也可以只传对应字段（例如 `prompt` / `templateId` / `goal` / `projectId`）。

- **Hint**: `destructive`

### 7.3 antigravity_trigger_scheduler_job
立即执行一次指定的定时任务，不影响后续 cron/interval 的正常触发。

- **Hint**: `destructive`

### 7.4 antigravity_delete_scheduler_job
永久删除指定的定时任务。
