...
启动服务后，在页面顶部命令框输入一句话，CEO Agent 自动完成**部门匹配 → 模板选择 → 项目创建 → 任务派发**：

```
让超级IT研发部写一个 Python 脚本计算 1+10 的结果
```
...
curl -X POST http://localhost:3000/api/ceo/command \
  ...
  -d '{"command": "让超级IT研发部写一个 Python 脚本计算 1+10 的结果"}'
```
...
| 方式 | 入口 | 适合 |
|:-----|:-----|:-----|
| **CEO 命令** | Header 中央命令框 / `POST /api/ceo/command` | 日常任务，一句话搞定 |
| **快速任务** | OPC 项目列表顶部输入框 | 选定部门 + 选模型 |
| **手动派发** | 创建项目 → 选模板 → 派发 / `POST /api/agent-runs` | 高级控制，指定模板和参数 |

### 系统架构一句话

```
CEO 命令 → 匹配部门 → 选择 Template → dispatch → Agent Worker 在隐藏对话中执行 → 输出结果
```
...
CEO Agent 是用户与整个 AI 组织之间的**智能路由器**。用户说一句话，CEO 自动完成：

```
"让研发部开发登录接口"
  │
  ├─ 识别部门 → 研发部 (workspace 匹配)
  │
  ├─ 选择模板 → coding-basic-template (语义评分)
  │
  ├─ 创建 Project → 记录目标和状态
  │
  └─ 派发 Run → Agent Worker 开始执行
```
...
{
  "command": "让研发部做一个用户登录模块",
  "options": {
    "model": "gemini-3-flash"      # 可选，指定模型
  }
}
```
...
```json
{
  ...
  "message": "已将任务「用户登录模块」派发给 研发部（简单编码），执行已启动",
  "projectId": "xxx",
  "runId": "yyy"
}
```
...
| Action | 触发示例 | 说明 |
|:-------|:---------|:-----|
| `create_project` | "让研发部做XX" | 创建项目并启动执行 |
| `multi_create` | "研发部和运营部一起做XX" | 跨部门协作，创建多个项目 |
| `cancel` | "取消XX项目" | 取消运行中的任务 |
| `pause` | "暂停XX" | 暂停项目 |
| `resume` | "恢复XX" | 恢复已暂停的项目 |
| `retry` | "重试XX" | 重新启动失败的任务 |
| `skip` | "跳过XX" | 跳过当前 Pipeline 阶段 |
| `needs_decision` | 无合适模板时 | 返回建议列表让 CEO 选择 |
| `report_to_human` | 无法匹配部门 | 提示指定部门 |

### 2.4 CEO 模板选择逻辑

CEO 按以下优先级自动选择模板：

1. **部门配置的 `templateIds`** — 如果部门明确配置了模板列表，优先从中选择
2. **全局模板语义评分** — 按任务描述 vs 模板标题/描述/角色关键词评分
3. **优先回退** — `universal-batch-template` > `coding-basic-template` > `adhoc-universal`

评分因素：模板标题匹配（+10）、描述关键词（+2）、组内角色关键词（+5）、有 pipeline stages（+2）、adhoc 惩罚（-5）。

### 2.5 CEO 负载保护

当部门活跃项目 ≥ 5 时，CEO 拒绝派发并提示"负载过高"。这是保护机制，避免 Agent 资源过载。
...
| 模板 ID | 名称 | Pipeline | 说明 |
|:--------|:-----|:---------|:-----|
| `adhoc-universal` | 🔧 通用兜底 | 0 stages | 单 Agent 直接执行，无 Pipeline 追踪。CEO 的最后回退选项 |
...
```json
{
  ...
  "groups": {
    "product-spec": {
      ...
      "maxReviewRounds": 3,
      "sourceContract": { "acceptedSourceGroupIds": [] }
    },
    "architecture-advisory": {
      ...
      "maxReviewRounds": 3,
      "sourceContract": { "acceptedSourceGroupIds": ["product-spec"] }
    },
    "autonomous-dev-pilot": {
      ...
      "sourceContract": { "acceptedSourceGroupIds": ["architecture-advisory"] }
    }
  },

  "pipeline": [
    { "groupId": "product-spec", "autoTrigger": true },
...
| `roles[]` | array | 角色列表 |
| `maxReviewRounds` | number | 最大审查轮数（review-loop 模式） |
| `sourceContract.acceptedSourceGroupIds` | string[] | 上游依赖（空 = 入口 group） |
...
| 参数 | 类型 | 说明 |
|:-----|:-----|:-----|
| `groupId` | string | 对应执行的 group |
| `stageId` | string? | 阶段标识（默认取 groupId） |
...
> **重要**：推荐只传 `templateId`（不传 `groupId`），系统自动从第一个 stage 开始，每阶段完成后自动触发下一阶段。
...
```bash
GET /api/agent-groups                      # 所有可用 Group
...
```json
{
  ...
  "skills": [
    { "skillId": "frontend", "name": "前端开发", "category": "frontend", "difficulty": "mid" }
  ],
...
| `skills` | 技能清单（CEO 按技能匹配任务） |
| `provider` | 默认执行 Provider（`antigravity` / `codex`） |
...
| Run/Project 元数据 | `~/.gemini/antigravity/gateway/agent_runs.json` / `projects.json` |
