...
1. `create-project`：日报任务项目、周报任务项目、周期性研究、ad-hoc 例行任务
2. `health-check`：项目健康巡检
3. `dispatch-pipeline`：固定模板 / 固定 stage 的定时派发

其中 `create-project` 会在创建 Scheduler Job 时优先尝试解析 templateId：先看显式模板名，再参考部门模板与任务语义，必要时回退到全局更合适的模板。若成功选出模板，触发时会“创建 Project + 自动派发第一条 run”；若没有唯一模板，则保留为“只创建 Project”。
