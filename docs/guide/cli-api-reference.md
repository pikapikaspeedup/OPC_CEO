...
- `Conversations / CEO Office chat` 现在也能在 Gateway 本地会话轨道下独立工作，不再只绑定 Antigravity IDE 会话
- `codex / claude-code` 若参与主线，会以 Claude Engine `ExecutionTool` 的形式出现，而不是新的 provider/backend。
...
- **Request Body (JSON):**
  ...
  - `governance.platformEngineering.observe` (Boolean, optional): Include this project in platform engineering observation
  - `governance.platformEngineering.allowProposal` (Boolean, optional): Allow platform engineering to auto-generate system-improvement proposals from real run failures
- **Example Body:**
  ```json
  {
    "name": "Tetris Game",
...
- **当前实现边界:**
  ...
  - `native-codex` 的 Department / agent-runs 主链已经切到 Claude Engine，并要求 provider 主线固定走 `pi-ai`；本地 conversation 也统一走 Claude Engine transcript-backed 路径。
...
- **URL:** `GET /api/company/loops/notification-targets`
...
- **Note:** `run-now` 执行 daily/weekly/growth/risk review，只选择 Top-N agenda，dispatch 仍经过 budget/circuit gate，policy disabled 时返回 skipped。scheduler 内置 daily/weekly company-loop 使用 cron job，不创建 5s interval，并从 `CompanyLoopPolicy` 读取 cadence / timezone / enabled。loop run 的 `metadata.skippedAgenda` 保存 skipped 原因；`notificationChannels` 会产出 web/email/webhook channel-specific notification id，其中 `web` 固定启用，`email/webhook` 仅在外部投递目标已接入时有效。
...
- **Note:** high/critical proposal 不能通过 passed test evidence 绕过 approval；`approve` 会写入持久 approval metadata，并自动创建平台工程 Project、派发 `development-template-1` 首跑，返回 `proposal + launch`；proposal 随平台工程项目状态和测试证据自动推进到 `in-progress/testing/ready-to-merge`；`GET` proposal 接口会返回 `exitEvidence.project/latestRun/testing/mergeGate`。
...
- **Description:** 手动创建 `manual` source 的知识资产；请求体支持 `title`、`summary`、`content`、`workspaceUri`、`category`、`tags[]`。
- **Response:** `201 Created` `KnowledgeDetail`

#### Knowledge Asset Detail
- **URL:** `GET /api/knowledge/:id`
- **Description:** 返回完整 `KnowledgeDetail`，包含 `artifacts` 以及 richer metadata：`tags`、`scope`、`sourceType`、`sourceRunId`、`confidence`、`evidenceCount`、`promotionLevel`。
...
## Provider Credentials And Execution Tool State API
...
# Inspect cached model catalog for a provider
curl -s "http://localhost:3000/api/provider-model-catalog?provider=native-codex" | jq

# Read key presence + local install/login status
curl -s http://localhost:3000/api/api-keys | jq

# Inspect local cc-connect installation / config / running state
curl -s http://localhost:3000/api/cc-connect/local-state | jq
...
# Trigger a provider-backed image generation smoke test
curl -sX POST http://localhost:3000/api/provider-image-generation \
  -H "Content-Type: application/json" \
  -d '{
...
# Generate and persist an AI summary for a Knowledge item
curl -sX POST http://localhost:3000/api/knowledge/d711d04b-2d86-4d2f-b5fe-7191230c50fd-pattern-512ee5fbe0/summary | jq
...
# Re-test a saved custom connection without re-entering the key
curl -sX POST http://localhost:3000/api/api-keys/test \
  -H "Content-Type: application/json" \
  -d '{
    "provider": "custom",
...
# Repair ~/.cc-connect/config.toml and inject [management]
curl -sX POST http://localhost:3000/api/cc-connect/manage \
  -H "Content-Type: application/json" \
  -d '{ "action": "prepare-config" }' | jq

# Start local cc-connect from Settings / CLI
curl -sX POST http://localhost:3000/api/cc-connect/manage \
  -H "Content-Type: application/json" \
  -d '{ "action": "start" }' | jq

# Stop local cc-connect
curl -sX POST http://localhost:3000/api/cc-connect/manage \
  -H "Content-Type: application/json" \
  -d '{ "action": "stop" }' | jq

# Read proxied cc-connect management status
curl -s http://localhost:3000/api/cc-connect/status | jq
```
...
- Legacy `codex / claude-code` values in `ai-config` are normalized to `native-codex`; local CLI coder is shown separately as execution tools, not provider choices.
- Claude Engine `coding / full` toolsets now bind a unified `ExecutionTool`; `codex / claude-code` are invoked through that tool contract rather than through provider selection.
- Existing invalid configs remain visible as `(...未配置)` placeholders until the user switches them to a valid provider and saves.
- `providerProfiles` now stores transport/auth/capability truth per provider; it does not change the scene/layer routing priority.
- `providerProfiles[provider].enabled = false` hides a detected local provider from this system without deleting its machine-wide login state.
- `customProviders[] + activeCustomProviderId` now express multiple OpenAI-compatible connections; runtime still materializes the active one into `customProvider` for backward compatibility.
- `GET/POST /api/provider-model-catalog` drives the Settings model picker and refresh flow; catalog snapshots are cached at `~/.gemini/antigravity/provider-model-catalog.json`.
- `custom.baseUrl` accepts either root URLs or `/v1` URLs; the backend normalizes both and no longer duplicates `/v1`.
- `POST /api/provider-image-generation` powers the Settings image-generation smoke test for configured image-capable providers; current built-in paths are `native-codex`、`openai-api`、`custom`，其中 `native-codex` 会把过小尺寸自动归一到 `1024x1024`。
- `POST /api/knowledge/:id/summary` reuses the `knowledge-summary` scene and the same provider/model routing truth used elsewhere in the system.
- `GET /api/cc-connect/local-state` and `POST /api/cc-connect/manage` are web-owned local-management endpoints for the `会话平台` tab; they do not proxy to cc-connect.
- `/api/cc-connect/[...path]` remains the thin proxy onto the cc-connect Management API once the local process is already running.
