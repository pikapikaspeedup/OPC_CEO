...
| 模板 ID | 标题 | Groups | 文件 |
|:--------|:-----|:-------|:-----|
| `development-template-1` | 完整产研链 | product-spec → architecture → dev | `.agents/assets/templates/development-template-1.json` |
| `design-review-template` | 产品体验评审 | ux-review | `.agents/assets/templates/design-review-template.json` |
| `ux-driven-dev-template` | 交互驱动产研 | ux-review → product-spec → arch → dev | `.agents/assets/templates/ux-driven-dev-template.json` |
| `coding-basic-template` | 简单编码 | coding-basic | `.agents/assets/templates/coding-basic-template.json` |

Workflow（角色指令文件 `.agents/workflows/*.md`）是可复用资产，可被多个模板引用。模板之间不共享 — 需要新场景就新建模板。
...
一次具体的任务执行。Run 有完整的生命周期：`queued → starting → running → completed/blocked/failed`。

### Workflow（工作流）

定义在 `.agents/workflows/` 下的 Markdown 文件，是 Agent Worker 的"执行脑"——告诉 AI 该做什么、按什么顺序做、最终输出什么。V3.5 强调围绕 Workflow 产出结构化文件。
...
├── runs/              ← 该项目的所有 run（例如 data/projects/{projectId}/runs/{runId}/）
└── integration/       ← 集成产物
```
...
```json
{
  "status": "completed", 
  "summary": "详细的任务执行总结摘要",
  "changedFiles": ["src/app.tsx", "data/projects/xxx/runs/yyy/specs/draft-spec.md"],
...
正确的文件路径请认准 **`data/runs/`** 和 **`data/projects/`**。这些文件不在 `.agents/` 下面。

### 9.1 Advisory Run 产物目录

如果你使用的是独立 Run，路径为 `data/runs/<runId>/`。如果在 Project 中，则是 `data/projects/<projectId>/runs/<runId>/`：

```
data/runs/<runId>/ (或者 data/projects/<projectId>/runs/<runId>/)
...
同样使用 `data/runs/` 或 `data/projects/.../runs/` 前缀：

```
data/projects/<projectId>/runs/<runId>/
├── task-envelope.json
...
### Q: 所有相关的数据存放位置在哪里？
在 V3.5 中：
- **元数据存档**：记录在根系统下的 `data/agent_runs.json` 及 `data/projects.json` 中。
- **运行时数据与产物目录**：项目内由 Runtime 管理至 `data/runs/<runId>/` 或 `data/projects/<projectId>/runs/<runId>/` 下。
- **Prompt/指令工作流存储**：仍保留在项目仓库的 `.agents/workflows/` 下。
- **模板配置**：`.agents/assets/templates/*.json` — 每个文件是一个完整的 Template（groups + pipeline）。
- **审查策略**：`.agents/assets/review-policies/*.json`。
