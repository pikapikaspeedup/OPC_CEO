...
- **legacy 注册表常量 + 迁移工具**：`gateway-home.ts` 的 `PROJECTS_FILE`/`RUNS_FILE`/`CONVS_FILE`/`SCHEDULED_JOBS_FILE` 此前由 `scripts/migrate-storage-to-sqlite.ts`（一次性 JSON→sqlite 迁移工具）使用，故并非死代码。**2026-05-30：项目处于测试阶段、无老装机需迁移，经确认整体退役——已删除迁移脚本 + 这 4 个常量**（保留仍在用的 `HIDDEN_WS_FILE`）。另：`listLegacyFilesystemKnowledgeIds` 及 deprecated 别名 `TemplateGroupSummary`/`TemplateGroupDetailFE`（均 0 引用）也已移除。
