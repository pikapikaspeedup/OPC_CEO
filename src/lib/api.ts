...
const FETCH_MAX_GET_ATTEMPTS = 3;

function fetchBackoff(attempt: number): Promise<void> {
...
async function fetchJson<T>(url: string, init?: RequestInit): Promise<T> {
  ...
  const maxAttempts = idempotent ? FETCH_MAX_GET_ATTEMPTS : 1;
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    ...
    try {
      ...
      // Network error / timeout — transient upstream blip (e.g. api restarting).
      lastError = error;
      if (idempotent && attempt < maxAttempts) {
        await fetchBackoff(attempt);
        continue;
      }
