...
/**
 * Optional analytics callback. Set via `configureAnalytics()` to receive
 * security check events without coupling to any specific analytics provider.
 */
let _logEvent: (eventName: string, data?: Record<string, unknown>) => void = () => {}

export function configureAnalytics(
  logFn: (eventName: string, data?: Record<string, unknown>) => void,
): void {
  _logEvent = logFn
}
...
/**
 * Tree-sitter analysis data (optional). When provided, enables more accurate
 * parsing. Defined inline to avoid depending on tree-sitter bindings.
 */
