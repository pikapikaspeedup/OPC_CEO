...
import { type Node, PARSE_ABORTED } from './parser'
import type { PermissionResult } from './PermissionResult'
import type { PermissionUpdate } from './PermissionUpdateSchema'
import { createPermissionRequestMessage } from './permissionsCore'
import { BashTool } from './toolTypes'
import { bashCommandIsSafe } from './bashSecurity'
...
async function bashToolCheckCommandOperatorPermissions(
  ...
  if (isUnsafeCompound) {
    ...
    const safetyResult = bashCommandIsSafe(input.command)
