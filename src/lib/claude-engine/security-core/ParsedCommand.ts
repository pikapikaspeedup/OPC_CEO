...
let _treeSitterAvailableCache: boolean | undefined;
const getTreeSitterAvailable = async (): Promise<boolean> => {
  if (_treeSitterAvailableCache !== undefined) return _treeSitterAvailableCache;
  try {
    const { parseCommand } = await import('./parser')
    const testResult = await parseCommand('echo test')
    _treeSitterAvailableCache = testResult !== null;
    return _treeSitterAvailableCache;
  } catch {
    _treeSitterAvailableCache = false;
    return false;
  }
}
