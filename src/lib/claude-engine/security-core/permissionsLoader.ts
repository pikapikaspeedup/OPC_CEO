// @ts-nocheck — extracted from decompiled code, type errors don't affect runtime
import { getSettingsForSource, updateSettingsForSource, getFsImplementation, getSettings_DEPRECATED } from './stubs'
type SettingsJson = Record<string, unknown>
type EditableSettingSource = string
const readFileContent = async (p: string) => { const fs = await import('fs/promises'); return fs.readFile(p, 'utf-8') }
const fileExists = async (p: string) => { try { const fs = await import('fs/promises'); await fs.access(p); return true } catch { return false } }
const readFile = async (p: string) => { const fs = await import('fs/promises'); return fs.readFile(p, 'utf-8') }
const safeReadJSON = async <T>(p: string, defaultVal: T): Promise<T> => { try { const fs = await import('fs/promises'); return JSON.parse(await fs.readFile(p, 'utf-8')) } catch { return defaultVal } }
const safeWriteJSON = async (p: string, data: unknown) => { const fs = await import('fs/promises'); await fs.writeFile(p, JSON.stringify(data, null, 2)) }
const logError = (...args: unknown[]) => console.error(...args)
import { getSettingSourceDisplayNameLowercase, SETTING_SOURCES } from './stubs'
import { getSettings_DEPRECATED } from './stubs'
const getSettingAsString = (_name: string, defaultValue: string = '') => defaultValue
// settings types stubbed
import type {
  PermissionBehavior,
  PermissionRule,
