...
graph TB
    ...
    subgraph Gateway["Antigravity Gateway (opc-web:3000 + opc-api:3101)"]
        ...
        CEOAgent["CEO Workspace<br/>Conversation playbooks"]
...
后文仍按系统层分章节展开，但如果要快速理解“现在这套系统到底靠什么能力站住”，最接近真实落地的入口是下面六条主线：
...
graph LR
    ...
    subgraph OPC["组织治理层"]
        CEOAgent["CEO Workflow<br/>Conversation + playbooks"]
...
15. 内置 `平台工程部` 作为系统默认自开发部门，workspace 固定在 `AG_GATEWAY_HOME/system-workspaces/platform-engineering/`；它复用现有 Department / Project / Company Kernel / Approval / Scheduler 机制，不新增第二套模型。平台工程部项目默认开启观察与自动提案治理字段。
...
graph TB
    subgraph IDE["IDE / Conversation 层"]
        IDESkill["IDE Skill<br/>AI 能力注册<br/>(由 Language Server 提供)"]
    ...
    IDESkill -.->|"不参与编排<br/>仅 IDE 对话"| ConvWF
    ConvWF -.->|"不同于"| Workflow
```
...
1. **IDE Skill ≠ Canonical Skill Asset ≠ Workflow**：IDE Skill 是 runtime 自带能力；Canonical Skill Asset 是 Gateway 资产；Workflow 是角色或 Prompt Mode 的可执行 playbook。
...
| 文件 | 职责 |
|---|---|
| `src/lib/bridge/codex-adapter.ts` | Codex CLI 封装：exec 函数 + MCP Client 类；会优先解析成熟 `codex-cli` 二进制，避免命中旧本地 `codex` 包 |
| `src/lib/platform-engineering-codex-runner.ts` | 平台工程部自开发执行脚手架：生成 checkpoint/snapshot 基线、创建 git worktree、调用 `codex exec --cd`、收集 scope / diff / validation evidence |
...
Provider Abstraction Layer 仍然负责“组织想选哪个 provider”的解析，但 `TaskExecutor` 已不再覆盖全部真实执行主线。

当前要分两层看：

1. `TaskExecutor / getExecutor()`
   - 现在主要承载 `antigravity` 与本机执行工具 adapter 的底层接线
   - `codex / claude-code` 在主线语义上已经不是 provider/backend，而是由 Claude Engine `ExecutionTool` 调用的执行工具
2. `AgentBackend / ClaudeEngineBackend`
   - 承担 `agent-runs` / Department runtime 主线
   - `native-codex / claude-api / openai-api / gemini-api / grok-api / custom` 全部走这里
   - Claude Engine session 会按 toolset 绑定统一的 `ExecutionTool` runtime，再去调用 `codex / claude-code`
   - `shell / file / search / MCP resources` 仍然是 Claude Engine 的普通工具，不属于 `ExecutionTool`

因此，PAL 更准确的定位是“provider resolution + Claude Engine 主线接线”，而不是“所有运行能力都直接实现同一个 TaskExecutor 接口”。
...
flowchart TB
    ...
    subgraph PAL["Provider Abstraction Layer"]
        ...
        BE["getBackend(providerId)<br/>ClaudeEngine / Antigravity"]

        subgraph Backends["主线 Backend"]
            AGB["AntigravityAgentBackend<br/>gRPC Child Conversation"]
            CEB["ClaudeEngineAgentBackend<br/>provider via pi-ai"]
        end

        subgraph ExecutionTools["Claude Engine ExecutionTool runtime (external coders only)"]
            ET["ExecutionTool<br/>list / run"]
            CE["CodexExecutor<br/>Codex CLI / MCP"]
...
1. `antigravity` 的底层执行入口
2. `ExecutionTool` 调用 `codex / claude-code` 时的 adapter contract

它不再代表所有 provider/backend 的统一主线抽象。
...
```json
{
  "scenes": {
    ...
    "nudge": { "provider": "native-codex", "constraints": { "timeout": 60000 } }
  }
}
```
...
业务层 provider 继续由 `defaultProvider / layers / scenes` 决定，但 transport/auth/capability 已拆到 `providerProfiles`：
...
1. `Native` 现在只代表 `Antigravity IDE/runtime` 这类平台专有路径。
2. 第三方 provider transport 不再把“我们手写 transport”也称为 Native。
3. `native-codex`、`claude-api`、`openai-api`、`gemini-api`、`grok-api`、`custom` 的运行时主链统一由 `Claude Engine` 编排，且 Claude Engine 主线固定走 `pi-ai`；不再在 Department / agent-runs / compaction 主线上回退到 provider-specific native transport。
...
| 标识 | 角色 | 实现类 | 协议 | 状态 |
|:-----|:-----|:-------|:-----|:-----|
| `antigravity` | AI Provider | `AntigravityExecutor` | gRPC → Language Server | ✅ 已实现 |
| `native-codex` | AI Provider | `ClaudeEngineAgentBackend` | OAuth → `pi-ai` transport（文本 / tools / 本地会话） | ✅ 已实现 |
| `claude-api` | AI Provider | `ClaudeEngineAgentBackend` | Anthropic API → `pi-ai` transport | ✅ 已实现 |
| `openai-api` | AI Provider | `ClaudeEngineAgentBackend` | OpenAI API → `pi-ai` transport | ✅ 已实现 |
| `gemini-api` | AI Provider | `ClaudeEngineAgentBackend` | Gemini REST API → `pi-ai` transport | ✅ 已实现 |
| `grok-api` | AI Provider | `ClaudeEngineAgentBackend` | xAI / Grok API → `pi-ai` transport | ✅ 已实现 |
| `custom` | AI Provider | `ClaudeEngineAgentBackend` | OpenAI-compatible API → `pi-ai` transport | ✅ 已实现 |
| `codex` | 执行工具 | `ExecutionTool -> CodexExecutor` | MCP → Codex CLI subprocess | ✅ 已实现 |
| `claude-code` | 执行工具 | `ExecutionTool -> ClaudeCodeExecutor` | Claude Code CLI → stream-json | ✅ 已实现 |

主线语义上，`codex / claude-code` 现在都属于 Claude Engine 可调用的 `ExecutionTool`，不是与 `ClaudeEngineAgentBackend` 平级的 provider/backend family。
...
`resolveProvider()` 选出的 provider 只是“用户/组织想用谁”的第一步，不一定是最终执行者。真正 dispatch 前还会做一次 capability-aware routing：

| 执行等级 | 代表含义 | 最低能力要求 | 轻量 provider 结果 |
|---|---|---|---|
| `light` | 轻任务 / 对话式执行 | 无 | 轻量 provider 可直接执行；必要时可由 Claude Engine 调用 `codex / claude-code` 这类执行工具 |
| `artifact-heavy` | 产物导向任务 | Department runtime + tool runtime + artifact contract + read/write audit | 轻量 provider 会被拒绝或升级 |
| `review-loop` | author / reviewer 闭环 | 上述能力 + review loop | 轻量 provider 会被拒绝或升级 |
| `delivery` | 交付类任务 | 上述能力 + permission enforcement | 轻量 provider 会被拒绝或升级 |

运行时会把 `requestedProvider / routedProvider / providerRoutingReason / requiredExecutionClass` 记录进 run provenance，便于复盘为什么发生了 provider 升级或回退。
...
它会在每个 Claude Engine session 的 `ToolContext` 上绑定默认 `ExecutionTool` runtime，因此 `coding / full` toolset 可以统一调 `codex / claude-code`，而不需要再切另一套 backend family。
...
1. `native-codex`
   ...
   - 本地 conversation 也统一走 `api-provider-conversations` / Claude Engine transcript
...
它们不属于 Department backend 主线，但已经通过 `ExecutionTool` 进入 Claude Engine 会话上下文；`/api/codex*` 与直连 backend 仍保留给手工/兼容入口，不作为自进化主路径的架构中心。

### 运行历史与会话持久化

| 执行路径 | 原始过程来源 | Cutover 后持久化 | 当前可见性边界 |
|:--------------|:-------------|:-----------------|:---------------|
...
| `codex` | Claude Engine `ExecutionTool` / 手工 Codex API transcript | `storage.sqlite` + `run-history.jsonl`（user/assistant/tool/result） | 新 run 可完整回放；老 run 通过迁移补出 transcript/result/verification/artifact |
| `native-codex` / `claude-api` / `openai-api` / `gemini-api` / `grok-api` / `custom` | Claude Engine transcript + tool/completion 事件 | `storage.sqlite` + `run-history.jsonl` | 新 run 按统一 schema 可回放；旧 `native-codex` 直连会话仅保留 run-history 兼容回放 |
| `claude-code` | Claude Engine `ExecutionTool` / 手工 Claude Code transcript | `storage.sqlite` + `run-history.jsonl` | 新 run 可回放；历史 run 取决于 cutover 前是否已有可导入 transcript |
...
- Provider 管理旅程已经收敛成：
  1. `AI 接入`：展示已接入、支持添加 / 编辑 / 复测 / 删除 / 恢复
  2. `默认配置`：只负责选择默认 Provider / 默认模型
  3. `本机执行工具`：只展示 `codex / claude-code` 这类 CLI 可调用能力，不进入 Provider 选择
- `/api/ai-config` 在保存前会对 `defaultProvider`、`layers.*.provider`、`scenes.*.provider` 做同一套校验，拒绝任何未配置 Provider，避免前端绕过。
- `/api/ai-config` 会把遗留的 `codex / claude-code` 配置值统一归一为 `native-codex`，避免旧配置继续污染 Provider 平面。
- `providerProfiles[provider].enabled` 主要影响 UI 可见性与配置校验；部分技术能力探测（如模型目录聚合）仍可能按“技术上可用”继续工作，因此“可发现”与“当前系统启用”必须分开理解。

### Provider 成本与可见性

Provider 平面里，“能不能调用”和“剩余额度能不能看见”是两件不同的事：

1. `/api/models`
   - 先尝试读取 Antigravity runtime 模型目录
   - 再与 provider model catalog fallback 合并
2. `/api/me`
   - 返回 `providerCredits`、`providerUsageSummary`、`creditSource`、`providerAwareNotice`
   - 当默认 provider 不是 `antigravity` 时，会明确提示“当前 credits 仍主要反映 Antigravity runtime”
3. `/api/analytics`
   - 优先合并 Antigravity runtime analytics
   - runtime 不可用时自动回退到 Gateway 本地 run 聚合
4. `run.tokenUsage`
   - 是 Gateway 对 provider 使用量的本地真相源
   - 能稳定做 usage aggregation 与预算估算，但不能替代所有上游 vendor 的剩余额度 API
5. `native-codex` / `custom`
   - 可稳定追踪本地 run usage
   - 但通常无法通过稳定公开 API 读取真实剩余额度，因此必须把“usage 聚合”和“credit 查询”分开建模
...
| `src/lib/providers/ai-config.ts` | 配置加载/缓存/持久化 + `resolveProvider()` 4 级解析；遗留 `codex / claude-code` 配置值归一到 `native-codex` |
...
| `src/lib/provider-model-catalog.ts` | AI Provider 模型目录发现、缓存与 `/api/models` fallback 聚合；不再把 CLI coder 当作 provider catalog |
| `src/lib/provider-image-generation.ts` | provider-backed image generation：当前主支持 `native-codex` / `openai-api` / `custom`；任意非 `openai-api` 的主 provider 失败时都可回退到 `openai-api` |
| `src/lib/providers/antigravity-executor.ts` | gRPC 子对话创建 + 消息发送 |
| `src/lib/claude-engine/tools/execution-tool.ts` | Claude Engine 统一执行工具合同：列出 / 调用 `codex`、`claude-code`，并隐藏单轮/多轮差异 |
| `src/lib/providers/codex-executor.ts` | `ExecutionTool` 背后的 Codex CLI adapter；MCP Client 池管理 + 同步任务执行 |
| `src/lib/platform-engineering-codex-runner.ts` | 平台工程部代码任务 runner；每次运行创建隔离 worktree，传递小 task packet，并把证据包写入平台工程部 workspace |
| `src/lib/bridge/native-codex-auth.ts` | Codex OAuth Token 读取 / JWT 过期检查 / 自动刷新 |
| `src/lib/bridge/native-codex-auth.ts` | `pi-ai/oauth` 外层的 Codex 凭据存储适配：共享 `~/.codex/auth.json`，负责读写与同步，不自实现 OAuth 协议 |
| `src/lib/bridge/native-codex-adapter.ts` | Native Codex helper：文本 / tool 调用统一走 `pi-ai`，图像生成保留 Codex Responses `image_generation` 专用接线 |
| `src/lib/claude-engine/api/pi-transport.ts` | Claude Engine API-backed provider 的 `pi-ai` 流式 transport 适配 |
| `src/lib/providers/claude-code-executor.ts` | `ExecutionTool` 背后的 Claude Code CLI adapter；stream-json 结果归一化入口 |
| `src/lib/backends/claude-engine-backend.ts` | ClaudeEngine 进程内执行器 + AgentBackend 适配；默认绑定 `ExecutionTool` runtime |
| `src/lib/backends/builtin-backends.ts` | 内置 backend 注册表；`codex / claude-code` 仅以 `Legacy*ManualBackend` 形式保留给兼容/手工入口 |
...
4. **M4 工具层**：以 6 类基础文件/命令/搜索原语为底座，并在默认 registry 上挂出 plan、task、skill、web、MCP resources，以及统一的 `ExecutionTool`。
5. **M5 权限层**：提供规则类型、规则字符串解析、MCP 工具前缀匹配、来源优先级、内存态 `PermissionChecker`，并支持 `auto-mode-classifier` 驱动的 `auto` 权限模式。
...
flowchart LR
    ...
    ApiTypesNode --> ApiPiNode["api/pi-transport.ts<br/>pi-ai model resolve / stream adapter / tool schema bridge"]
    ApiErrorNode["api/api-client-error.ts<br/>normalized API client error"] --> ApiPiNode
    ToolRegistry --> ToolSchemaNode["api/tool-schema.ts<br/>Tool -> APITool / JSON Schema"]
    TypesIndex --> ToolSchemaNode
    ApiErrorNode --> ApiRetryNode["api/retry.ts<br/>pi-ai only routing / 指数退避 / jitter / retry event"]
    ApiTypesNode --> ApiRetryNode
    ApiTypesNode --> ApiUsageNode["api/usage.ts<br/>usage 累加 / 费用估算"]
    ApiRoutingTestsNode["api/__tests__/pi-transport-routing.test.ts<br/>主线 pi-ai routing / no fallback"] --> ApiPiNode
    ApiRoutingTestsNode --> ApiRetryNode
    ApiErrorTestsNode["api/__tests__/errors-and-caching.test.ts<br/>error classify / cache helper"] --> ApiErrorNode
    ApiProviderFallbackTestsNode["api/__tests__/provider-fallback.test.ts<br/>provider chain / detectProvider"] --> ApiRetryNode
    ...
    ApiRetryNode --> QueryLoopNode["engine/query-loop.ts<br/>turn loop / tool_result user message / token budget"]
...
5. M5 当前主线已覆盖规则解析、MCP 前缀匹配、来源优先级、mode 判定和 `auto-mode-classifier`；但仍不包含完整权限 UI、磁盘持久化策略中心或 denial tracking。
6. M6 当前主线只保留 `pi-ai` transport、统一 retry/error/tool-schema/usage；旧的 Anthropic/OpenAI/Gemini/Grok 直连 transport shim 已删除，不再作为运行时入口。
7. M6 当前不实现 Bedrock/Vertex 鉴权适配，也不依赖 `@anthropic-ai/sdk` 或 `zod-to-json-schema`。
...
12. M4 的最底层原语仍是文件/命令/搜索 6 类基础工具，但默认 registry 已额外挂上 plan-mode、skill、skill-manage、task、todo、web、session-search、MCP resources 和 `ExecutionTool` 等高层工具；MCP 工具仍通过 M7 管理器桥接。
...
| `src/lib/claude-engine/api/api-client-error.ts` | 提供统一的 `APIClientError`，供 `pi-ai` transport 与错误分类层复用 |
| `src/lib/claude-engine/api/pi-transport.ts` | 负责 Claude Engine API-backed provider 的 `pi-ai` 流式 transport、模型解析与事件归一化 |
...
| `src/lib/claude-engine/api/__tests__/pi-transport-routing.test.ts` | 锁定 API-backed provider 主线固定走 `pi-ai`，且不再 native fallback |
| `src/lib/claude-engine/api/__tests__/errors-and-caching.test.ts` | 锁定 M6 错误分类、连接错误与缓存 helper |
| `src/lib/claude-engine/api/__tests__/provider-fallback.test.ts` | 锁定 provider fallback chain 与 `detectProvider` 行为 |
...
- **电脑 = 总部**，**workspace catalog = 组织名录**，**工作区目录 = 最小执行单元**，**Department = 可绑定多个工作区的治理对象**
- **CEO（用户）** 通过自然语言下达指令，系统先路由到 Department，再由 Department contract 决定 provider、工具集、运行边界
- **部门自运营**：自主选择 Provider、工具、工作方式，但最终仍受 runtime contract、预算 gate、审批和熔断保护

### Workspace Catalog 与 Department 边界

文件夹仍然是系统的核心执行载体，但“部门”已经不再等于“单个文件夹”：

1. `workspace catalog`
   - 把可管理 workspace 记入 SQLite
   - 来源包括 `antigravity-recent`、`manual-import`、`ceo-bootstrap`
   - `/api/workspaces/import` 只注册目录，不启动 runtime
2. `DepartmentConfig`
   - 仍落在 `workspace/.department/config.json`
   - 但可通过 `workspaceBindings[]` 把多个工作区绑定成一个 Department
3. `primary / execution / context`
   - `primary` workspace 决定 group key 与默认写入根
   - `execution` workspace 可参与协作执行
   - `context` workspace 默认只读，用作额外上下文
4. `executionPolicy.contextDocumentPaths`
   - 允许把部门说明、SOP、外部材料显式纳入 Department runtime 的 read roots

### Workspace 生命周期

同一个 workspace 在系统里至少有四种不同生命周期动作，不能混成“打开/关闭”两个按钮：

1. `import`
   - 只登记 catalog
   - 不启动 Antigravity runtime
2. `launch`
   - 先登记 catalog，再拉起 Language Server / runtime
3. `close`
   - 只隐藏 UI 并尝试最小化窗口
   - hidden 状态单独保存在 `hidden_workspaces.json`
4. `kill`
   - 真正终止 Language Server / 相关进程
   - 属于 runtime 行为，不是 control-plane 的可见性操作
...
interface DepartmentConfig {
  departmentId?: string                  // 逻辑部门 ID；多个 workspace 共享同一部门时复用
  name: string                          // 部门名称
  ...
  workspaceBindings?: Array<{           // 一个 Department 可绑定多个工作区
    workspaceUri: string
    role: 'primary' | 'execution' | 'context'
    alias?: string
    writeAccess?: boolean
  }>
  executionPolicy?: {
    defaultWorkspaceUri?: string        // 默认执行工作区
    contextDocumentPaths?: string[]     // 额外上下文文档
  }
}
```

平台工程部是一个内置的 Department 实例：

1. `departmentId = department:platform-engineering`
2. workspace 固定为 `AG_GATEWAY_HOME/system-workspaces/platform-engineering/`
3. 基础记忆放在 `.department/memory/shared|codex|claude-engine/`
4. 通过 `contextDocumentPaths` 注入自进化和架构文档
5. 默认模板集合包含 `development-template-1` 与 `coding-basic-template`
6. 代码级自开发任务通过 `platform-engineering-codex-runner` 创建目标仓库 git worktree；worktree 记录在 `worktrees/`，准出 evidence 写入 `evidence/codex-runs/`

### Project 治理字段

`ProjectDefinition` 当前新增：
...
1. `observe = true`：平台工程部观察该项目的真实运行。
2. `allowProposal = true`：观察到失败 run 时，可自动生成系统改进 proposal。
3. 平台工程部自身 workspace 创建出来的项目默认自动开启这两个字段。

### Department Runtime Contract

`DepartmentRuntimeContract` 是部门真正传给 backend 的运行边界，而不是 UI 上的描述性配置：

```typescript
interface DepartmentRuntimeContract {
...
CEO Office 的自然语言主入口是 Conversation。CEO workspace 的 identity / playbook 负责把用户意图转换为标准 API 或 MCP 工具调用，而不是维护第二条一次性命令解析链：

1. **对话入口**: CEO Office 通过 `/api/conversations` 创建或继续 CEO workspace 会话。
2. **调度入口**: 定时任务使用 MCP scheduler 工具，或 REST fallback `/api/scheduler/jobs`。
3. **执行入口**: 即时任务先 `POST /api/projects` 创建 Ad-hoc Project，再 `POST /api/agent-runs` 派发 run，并携带 `triggerContext.source = "ceo-workflow"`。

```mermaid
flowchart LR
    CEO["CEO Office<br/>Conversation"] --> Playbook["CEO workspace<br/>identity + playbooks"]
    Playbook --> Scheduler["定时调度<br/>MCP / scheduler REST"]
    Playbook --> Project["Ad-hoc Project<br/>projects REST"]
    Project --> Run["任务派发<br/>agent-runs REST"]
    Playbook --> Control["运行干预<br/>agent-runs intervene"]
    Playbook --> Report["状态查询<br/>management + projects + decisions"]
```

| 标准入口 | 说明 |
|:----------|:-----|
| `/api/conversations` | CEO Office 对话创建 / 列表 |
| `/api/conversations/:id/send` | CEO 对话续写 |
| `/api/scheduler/jobs` | 创建和管理 scheduler job，CEO workflow 应写入 `createdBy` 与 `intentSummary` |
| `/api/projects` + `/api/agent-runs` | 即时任务的 Ad-hoc Project + run 派发 |
| `/api/agent-runs/:id/intervene` | 运行中的任务干预 |
...
当前系统已经不是“一个部门一个 tokenQuota”这么简单，而是三层治理叠加：

1. `Department tokenQuota`
   - 位于 `.department/config.json`
   - 主要表达部门级软配额和 UI/运营约束
...
3. `Provider usage analytics`
   ...
   - 负责回答“最近 30 天哪个 provider 用了多少 token / 成功多少次 / 失败多少次”
4. `Process audit`
   - `storage.sqlite`、`run-history.jsonl`、`ops_audit/*.jsonl`、approval events、session provenance 共同组成复盘链路
   - 能追溯“谁在何时以哪个 provider 运行、为什么被预算 gate / approval / circuit breaker 拦下”
...
| `src/lib/company-kernel/ceo-decision-control.ts` | 聚合 CEO 决策队列，输出 `DecisionItemView[]` |
...
| `src/lib/approval/approval-urls.ts` | 基于 `DecisionTarget` 的审批详情深链 / signed feedback URL 生成 |
...
| `src/app/api/company/ceo/decisions/route.ts` | CEO 决策控制面统一列表 API |
...
| `src/server/control-plane/routes/workspaces.ts` | workspace catalog 列表 / import / close-hide 入口 |
...
安全相关代码现在分成“通用框架”和“真实主执行链”两套：

1. `src/lib/security/*`
   - 是通用 / 预备型安全框架
   - 提供 `checkToolSafety()`、generic permission modes、hook runner、sandbox manager 等抽象
2. `Claude Engine 当前主执行链`
   - `engine/tool-executor.ts` 先做 DepartmentRuntime policy + roots 校验
   - 再走 `PermissionChecker`
   - Bash 额外通过 `bash-security-adapter.ts` / `security-core` 判定

因此，`checkToolSafety()` 不是所有 Agent run 的统一入口；对 Claude Engine 来说，真实主线是 `ToolExecutor + PermissionChecker + roots + bash security`。

### 双栈安全边界

这里有两个安全栈同时存在，不能混成一个概念：

1. **IDE / CLI 原生安全栈**
   - `antigravity` 走 Language Server 自带文件权限与工具审批
   - `codex` 走 Codex CLI / MCP session sandbox
   - `claude-code` 走 Claude Code CLI 自身的权限/输出模型
2. **Gateway 托管安全栈**
   - 主要服务 `ClaudeEngineBackend` 及其工具运行时
   - `DepartmentRuntimeContract` 先定义读写根、artifact 根、permission mode、MCP server
   - 然后 `SecurityGuard -> PermissionEngine -> HookRunner -> SandboxManager` 再做二次约束
3. **multi-workspace 部门语义**
   - Gateway 安全判断的有效作用域不再只是“当前 workspace 根”
   - 还包括 `additionalWorkingDirectories / readRoots / writeRoots / artifactRoot`
   - 这正是多工作区部门仍能保持文件边界可审计的关键
4. **Web 端 IDE 权限审批仍未闭环**
   - `grpc.ts` 已补 `toolConfig` 里的 `code/viewFile/grep` 选项
   - 但 Web 侧仍没有完整实现 `HandleCascadeUserInteraction` / `GetUserSettings` / `SetUserSettings` 对应的 WAITING 审批闭环
   - 因此 IDE 原生权限链与 Gateway 本地权限链在交互能力上仍不对称

### 架构

```
┌─────────────────────────┐
│   Security Guard        │ ← generic / 预备型入口 checkToolSafety()
...
| 模式 | 行为 |
|:-----|:-----|
| `default` | 按规则评估，未匹配时通常进入 ask / deny 路径 |
| `acceptEdits` | 对常规编辑更宽松，但仍受规则和 roots 约束 |
| `bypassPermissions` | 尽量跳过权限判定，保留最底层技术约束 |
| `dontAsk` | 不走交互式审批，未明确允许时直接拒绝 |
| `plan` | 计划态；主要限制写入类工具 |
| `auto` | 结合 `auto-mode-classifier` 与规则做自动判定 |

说明：

1. 上表反映的是 Claude Engine 当前活跃权限模式。
2. `src/lib/security/types.ts` 里的 `strict/permissive/bypass/default` 仍保留为 generic framework 抽象，但不是当前主执行链的唯一模式集合。
...
5. **当前主线更偏 roots-based isolation** — Claude Engine 真实执行链当前更依赖 `readRoots / writeRoots / cwd` 和 bash 安全校验；完整 generic sandbox manager 仍偏预备态，而不是唯一生效层。
...
| `name` | 任务名称 |
| `cron` | Cron 表达式 |
| `timeZone` | 时区；company-loop / digest 类任务会显式使用 |
| `workspace` | 目标 workspace |
| `templateId` | Pipeline 模板 |
| `prompt` | 执行 prompt |
| `promptAssetRefs` / `skillHints` | Prompt Mode 资产提示，驱动 workflow-run + retrieval |
| `executionProfile` | 显式指定 `workflow-run / review-flow / dag-orchestration` |
| `enabled` | 是否启用 |
...
`Settings -> 会话平台` 现在分两层能力：

1. **本地管理**
   - `GET /api/cc-connect/local-state`
   - `POST /api/cc-connect/manage`
   - 负责安装探测、默认配置修复、启动、停止
2. **运行态代理**
   - `/api/cc-connect/[...path]`
   - 代理到本机 `cc-connect` Management API（默认 `127.0.0.1:9820/api/v1/*`）
   - 负责状态、项目、会话、heartbeat、cron 等运行态读取/操作

这两层故意拆开：

- 本地管理不依赖 cc-connect 已经启动
- 运行态代理只在 cc-connect 已经起来后才有意义
- Web 端可以直接把“修复默认配置 / 启动 cc-connect / 停止”做成一条完整用户旅程，而不是只给断开提示

`local-state` 返回的不只是 `running`，还会综合：
...
4. `managementEnabled`
5. `running`
6. `binaryPath / configPath / pid / issues`

`manage(action='prepare-config')` 会把模板修复到 `~/.cc-connect/config.toml`，因此“可管理”不等于“已经连上微信”。
...
| `/api/projects` | GET / POST | Project | 列表 / 创建项目；GET 支持 `page/pageSize` 分页 envelope；POST 允许附带 `governance.platformEngineering.observe/allowProposal` |
| `/api/projects/{id}` | GET / PATCH / DELETE | Project | 项目 CRUD；PATCH 允许更新平台工程治理字段 |
...
| `/api/workspaces` | GET | Control Plane | 工作区 catalog 列表 |
| `/api/workspaces/import` | POST | Control Plane | 导入 workspace 到 OPC catalog，不启动 runtime |
...
| `/api/me` | GET | Runtime | 用户信息 + providerCredits + providerUsageSummary + providerAwareNotice |
| `/api/ai-config` | GET / PUT | Control Plane | 组织级 provider 配置 |
| `/api/provider-model-catalog` | GET / POST | Control Plane | provider 模型目录读取/刷新；Settings 模型选择器与 `/api/models` fallback 使用 |
| `/api/provider-image-generation` | POST | Control Plane | Settings 图像生成 smoke test；当前主支持 `native-codex`、`openai-api`、`custom`；任意非 `openai-api` 的主 provider 失败时可回退到 `openai-api`；`native-codex` 会将过小尺寸归一到 `1024x1024` |
| `/api/knowledge/[id]/summary` | POST | Web App Route | 通过 `knowledge-summary` scene 生成并回写 Knowledge 摘要 |
...
| `/api/company/ceo/decisions` | GET | Company Kernel | CEO 决策控制面统一列表，返回 `DecisionItemView[]`；每项必须带 `DecisionTarget` |
...
| `/api/company/self-improvement/signals` | GET / POST | Company Kernel | SystemImprovementSignal 列表与创建；平台工程部会自动把 observed project 失败 run、文件级 `[不支持]` User Story gap，以及每日 Top 3 story-level candidates 同步到该信号池 |
| `/api/company/self-improvement/proposals` | GET | Company Kernel | SystemImprovementProposal 列表；返回 runtime-synced `exitEvidence`，以及派生 `controlState` / `entryApprovalSummary` |
| `/api/company/self-improvement/proposals/generate` | POST | Company Kernel | 从 signals 生成带风险、测试计划、回滚计划的 proposal；`story-top` candidate 会直接生成 `approval-required` proposal 并创建 CEO 准入审批请求 |
| `/api/company/self-improvement/proposals/{id}` | GET | Company Kernel | 系统改进 proposal 详情；直接聚合准入审批事实和统一控制面读模型 |
| `/api/company/self-improvement/proposals/{id}/evaluate` | POST | Company Kernel | 重新评估 protected core 风险并按需创建 approval request |
| `/api/company/self-improvement/proposals/{id}/approve` / `reject` | POST | Company Kernel | 更新系统改进 proposal 审批状态；`approve` 会自动创建平台工程 Project，并派发首个 self-improvement Codex tracking run，进入 direct Codex worktree/evidence/release-gate 链 |
...
| `/api/skills` | GET | Skill | canonical 技能列表 |
| `/api/skills/{name}` | GET / PUT / DELETE | Skill | canonical 技能详情 / 保存 / 删除 |
| `/api/skills/discovered` | GET | Skill | 从 IDE runtime 聚合 discovered skills 视图 |
| `/api/workflows` | GET | Workflow | canonical workflow 列表 |
| `/api/workflows/{name}` | GET / PUT / DELETE | Workflow | canonical workflow 详情 / 保存 / 删除 |
| `/api/workflows/discovered` | GET | Workflow | 从 IDE runtime 聚合 discovered workflows 视图 |
| `/api/rules` | GET | Rule | canonical 规则列表 |
| `/api/rules/{name}` | GET / PUT / DELETE | Rule | canonical 规则详情 / 保存 / 删除 |
| `/api/rules/discovered` | GET | Rule | 从 IDE runtime 聚合 discovered rules 视图 |
| `/api/analytics` | GET | Analytics | 优先返回 runtime analytics，并补充 Gateway provider usage 聚合；runtime 不可用时自动降级 |
| `/api/cc-connect/local-state` | GET | cc-connect | 本机安装 / 配置 / 绑定 / 运行状态探测 |
| `/api/cc-connect/manage` | POST | cc-connect | 修复默认配置、启动、停止 |
| `/api/cc-connect/{path}` | GET / POST | cc-connect | 代理本机 Management API 运行态接口 |
...
| `/api/approval` | GET / POST | Control Plane | 审批请求列表 / 提交；POST 必须提供 `target: DecisionTarget` |
